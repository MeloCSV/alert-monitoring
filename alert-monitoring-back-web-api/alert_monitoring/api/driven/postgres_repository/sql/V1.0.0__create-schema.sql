...
COMMENT ON TABLE default_alert_app IS 'Catálogo canónico de alertas por defecto. Una fila por tipo de alerta, independientemente de la nube o fichero Prometheus de origen.';
COMMENT ON COLUMN default_alert_app.raw_name IS 'Nombre original de la alerta en Prometheus';
COMMENT ON COLUMN default_alert_app.display_name IS 'Nombre legible para la UI';
...
COMMENT ON COLUMN default_alert_app.included_namespaces IS 'Unión de todos los patrones namespace=~ de las reglas críticas (re-inclusiones)';
...
CREATE INDEX idx_catalog_apps_csw_code ON catalog_apps (csw_code);
...
COMMENT ON TABLE alert_api IS 'Reglas de alerting expuestas por las APIs de Kibana. Se pueblan en el sync de Kibana y se consultan agrupadas por API.';
COMMENT ON COLUMN alert_api.rule_id IS 'ID original de la regla en Kibana';
COMMENT ON COLUMN alert_api.apis_alertadas IS 'Lista de APIs detectadas en el KQL o termField de la regla';
COMMENT ON COLUMN alert_api.message IS 'Mensaje descriptivo de la regla extraído de annotations.message en Kibana';
...
COMMENT ON COLUMN default_alert_api.raw_name IS 'Nombre técnico original de la alerta';
...
COMMENT ON COLUMN catalog_app_api.app IS 'Nombre canónico de la aplicación';
